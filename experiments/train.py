...
# Import AMP modules for mixed precision training
...
parser.add_argument("--lr", type=float, default=2.5e-5, help="Learning rate") # default was 1e-4
        ...
        if sep_query_head:  # TODO: this is different for each model type
            ...
            if "q_proj.weight" in name:
                ...
                if layer_id % kb_token_layer_frequency == 0:
                    old_weight = param.detach().clone()  # Clone to avoid in-place operations
            if "q_proj_new.weight" in name:
                layer_id = int(re.search(r"\d+", name)[0])  # type: ignore
                if layer_id % kb_token_layer_frequency == 0:
                    # Instead of copy_, set the data directly which doesn't require grad
                    with torch.no_grad():
                        param.data.copy_(old_weight)  # This doesn't affect the autograd graph
...
class Trainer:
    def __init__(
        ...
        llm_savename: str = "medical_llm",
        use_mixed_precision: bool = False, # New parameter for mixed precision
    ):
        ...
        self.use_mixed_precision = use_mixed_precision # Store the mixed precision flag
        
        # Initialize gradient scaler for mixed precision training
        self.scaler = GradScaler(init_scale=2**10) if self.use_mixed_precision else None
    ...
    def train(
        ...
        with create_custom_progress_bar(console=console) as pbar:
            ...
            for step in range(start_step, self.num_steps, 1):
                # Only zero out the gradients at the beginning of each full step, not each accumulation step
                self.optim.zero_grad(set_to_none=True)
                losses = []

                # Accumulate gradients
                for a_step in range(grad_accum_steps):
                    # Clear gradients for each batch in accumulation to prevent graph reuse issues
                    self.optim.zero_grad(set_to_none=True)
                    ...
                    if self.use_mixed_precision:
                        with autocast():
                            ...
                            EPS = 1e-8
                            loss = (loss_fct(shift_logits, shift_labels) * weights.max() / (weights + EPS)).mean()

                            # Scale the loss for backpropagation
                            self.scaler.scale(loss).backward()
                    else:
                        # Original code path without mixed precision
                        out = self.model(
                            input_ids=input_ids,
                            attention_mask=attention_masks,
                            kb_kvs=kb_embedding,
                        ...
                        EPS = 1e-8
                        loss = (
                            loss_fct(shift_logits, shift_labels) * weights.max() / (weights + EPS)).mean().mean()  # Make sure each sample is equally weighted
                ...
                if self.use_mixed_precision:
                    # Update with the scaler for mixed precision
                    self.scaler.unscale_(self.optim)  # Unscale gradients for optimizer step
                    # Clip gradients to prevent exploding gradients
                    torch.nn.utils.clip_grad_norm_(
                        chain(self.kbretriever.encoder.parameters(), 
                              self._get_params(self.model, self.sep_query_head, self.kb_token_layer_frequency)
                        ), 
                        max_norm=0.1
                    )
                    ...
                    # Clip gradients for standard training as well
                    torch.nn.utils.clip_grad_norm_(
                        chain(self.kbretriever.encoder.parameters(), 
                              self._get_params(self.model, self.sep_query_head, self.kb_token_layer_frequency)
                        ), 
...
def main():
    ...
    trainer = Trainer(
        ...
        model_save_dir,
        sep_query_head,
        use_mixed_precision=args.mixed_precision, # New parameter for mixed precision
    )
