...
def apply_rotary_pos_emb(query_states, key_states, cos, sin):
    ...
    assert query_states.shape[-2:] == cos.shape[-2:], "Mismatch in query_states and cos dimensions"
    assert key_states.shape[-2:] == sin.shape[-2:], "Mismatch in key_states and sin dimensions"
    
    # Apply rotary embeddings
    return (
        (query_states * cos) + (query_states.flip(-1) * sin),
        (key_states * cos) + (key_states.flip(-1) * sin),
    )